...
//   SkipGamepieceCommand skipGamepieceAuto{&}
...
class RobotContainer {
 public:
  ...
  ClimbCommand climb{&elevator, &shooter};
  LegAvoidanceCommand legAvoidance{&swerve};
  MiddleNoteDetectionCommand middleNoteDetectionBottom{&noteDetection, &swerve, &indexer, &limelight, &gyro, frc::Translation2d{8.3_m, 0.77_m}};
  MiddleNoteDetectionCommand middleNoteDetectionMiddleBottom{&noteDetection, &swerve, &indexer, &limelight, &gyro, frc::Translation2d{8.3_m, 2.45_m}};
 ...
 // frc2::InstantCommand stopShootAndIntake{[this](){shooter.SetVelocityKickerWheel(0); shooter.SetVelocityFlyWheel(0); indexer.SetPercent(0); shooter.SetPositionPivot(40); intake.SetPercent(0); indexer.SetPercent(0);}, {&shooter}}; 
  ...
  std::unordered_map<std::string, std::shared_ptr<frc2::Command>> buttonActionMap 
  {
    ...
    {"angleOffsetPositive", std::make_shared<frc2::InstantCommand>(angleOffsetPositive)},
    {"angleOffsetNegative", std::make_shared<frc2::InstantCommand>(angleOffsetNegative)},
    {"AmpExtend", std::make_shared<AmpExtendCommand>(ampExtend)},
    {"AmpRetract", std::make_shared<AmpRetractCommand>(ampRetract)},
    {"Climb", std::make_shared<ClimbCommand>(climb)},
    {"ClimbRetract", std::make_shared<frc2::InstantCommand>(climbRetract)},
    {"ClimbReset", std::make_shared<ClimbResetCommand>(climbReset)},
    {"humanPlayerSignalOn", std::make_shared<frc2::InstantCommand>(humanPlayerSignalOn)},
